...
CFLAGS=-v3 -O4 --c99 --keep_asm --display_error_number --endian=little --hardware_mac=on --obj_directory=$(GEN_DIR) --pp_directory=$(GEN_DIR) -ppd -ppa
...
OBJECTS=$(patsubst %,$(GEN_DIR)/%,$(SOURCES:.c=.object)) $(patsubst %,$(GEN_DIR)/%,$(ASM_SOURCES:.asm=.object))
